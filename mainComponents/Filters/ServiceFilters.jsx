...
export default function ServiceFilters({
  ...
  const categories = Array.isArray(filterOptions?.categories)
    ? filterOptions.categories.filter(Boolean)
    : []
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              ...
              <button
                ...
                className={`h-9 px-3 border rounded-lg text-xs font-semibold transition-all duration-300 flex items-center gap-1.5 whitespace-nowrap ${
                  showPopularOnly
                    ? "bg-primary/10 border-primary/30 text-primary ring-1 ring-primary/20"
                    : "bg-muted/30 border-border/50 text-foreground hover:bg-muted/50 hover:border-primary/20"
                }`}
              >
                <Star className={`w-3 h-3 ${showPopularOnly ? "fill-primary text-primary" : "text-muted-foreground/50"}`} />
                Popular
              </button>
          ...
          {hasActiveFilters && (
            <div className="flex items-center gap-2 flex-wrap animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              {selectedCategory !== "all" && (
                <Badge variant="secondary" className="text-[9px] px-2 py-0.5 bg-primary/10 text-primary border-0 rounded-full font-bold gap-1 hover:bg-primary/15 transition-colors">
                  {selectedCategory}
                  <button onClick={() => onCategoryChange("all")} className="hover:text-primary/60">
                    <X className="w-2.5 h-2.5" />
                  </button>
                </Badge>
              )}
              {showPopularOnly && (
                <Badge variant="secondary" className="text-[9px] px-2 py-0.5 bg-primary/10 text-primary border-0 rounded-full font-bold gap-1 hover:bg-primary/15 transition-colors">
                  ...
                  Popular
                  <button onClick={onPopularToggle} className="hover:text-primary/60">
                    <X className="w-2.5 h-2.5" />
                  </button>
                </Badge>
              )}
