...
import {
  ...
  ChevronDown, Briefcase, Users, Check
} from "lucide-react"
...
export default function TeamFilters({
  ...
  const departments = Array.isArray(filterOptions?.departments) ? filterOptions.departments.filter(Boolean) : []
  const roles = Array.isArray(filterOptions?.roles) ? filterOptions.roles.filter(Boolean) : []
          ...
          {hasActiveFilters && (
            <div className="flex items-center gap-2 flex-wrap animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              {selectedRole !== "all" && (
                <Badge variant="secondary" className="text-[9px] px-2 py-0.5 bg-primary/10 text-primary border-0 rounded-full font-bold gap-1 hover:bg-primary/15 transition-colors">
                  {selectedRole}
                  <button onClick={() => onRoleChange("all")} className="hover:text-primary/60">
                    <X className="w-2.5 h-2.5" />
                  </button>
                </Badge>
              )}
